...
export default function WazuhSIEMDashboard() {
  ...
  const escalateToSOAR = async (alert: Alert | Incident) => {
    try {
      // Prepare payload for Tracecat SOAR
      const payload = {
        alert_name: 'title' in alert ? alert.title : alert.description,
        severity: alert.severity,
        source: 'source' in alert ? alert.source : 'SIEM',
        rule: 'category' in alert ? alert.category : 'Incident Escalation',
        timestamp: 'timestamp' in alert ? alert.timestamp : alert.created,
        host: 'affected_hosts' in alert ? alert.affected_hosts.join(', ') : alert.source,
        ip: 'N/A',
      ...
      if (response.ok) {
        ...
        if ('id' in alert && 'description' in alert) {
          ...
          setIncidents(prev => prev.map(i => 
            i.id === alert.id ? { ...i, status: 'escalated' } : i
          ));
        }
      } else {
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold">Wazuh SIEM Platform</h1>
              <p className="text-gray-400">Tier 2 Analyst - Security Information & Event Management</p>
            </div>
          </div>
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
              ...
              <div className="space-y-3">
                {alerts.filter(a => a.severity === 'critical').slice(0, 3).map(alert => (
                  <div key={alert.id} className="bg-gray-900 border border-red-500/30 rounded-lg p-3">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <p className="font-semibold text-white">{alert.description}</p>
                        <p className="text-sm text-gray-400 mt-1">{alert.source} • {alert.id}</p>
                      </div>
                      <span className="px-2 py-1 text-xs rounded bg-red-500/10 text-red-400 border border-red-500/30 whitespace-nowrap">
                        CRITICAL
                      </span>
                    </div>
                  </div>
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
              ...
              <div className="space-y-3">
                {correlations.map(corr => (
                  <div key={corr.id} className="bg-gray-900 border border-gray-600 rounded-lg p-3">
                    <div className="flex items-start justify-between mb-2">
                      <p className="font-semibold text-white">{corr.name}</p>
                      <span className="px-2 py-1 text-xs rounded bg-blue-500/10 text-blue-400 whitespace-nowrap">
                        {corr.confidence}% confidence
                      </span>
                    </div>
                    <p className="text-sm text-gray-400">{corr.pattern}</p>
                    <p className="text-xs text-gray-500 mt-2">{corr.events} events over {corr.timespan}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              {Object.entries(mitreTechniques).map(([id, name]) => (
                <div key={id} className="bg-gray-900 border border-gray-600 rounded-lg p-3 hover:border-purple-500/50 transition-all cursor-pointer">
                  <p className="text-purple-400 font-mono text-sm">{id}</p>
                  <p className="text-white text-sm mt-1">{name}</p>
                  <p className="text-xs text-gray-500 mt-2">{Math.floor(Math.random() * 5) + 1} detections</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full bg-gray-900 border border-gray-600 rounded-lg pl-10 pr-4 py-2 text-white focus:outline-none focus:border-red-400"
                />
              </div>
              <select
                value={selectedSeverity}
                onChange={(e) => setSelectedSeverity(e.target.value)}
                className="bg-gray-900 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-red-400"
              >
          ...
          <div className="space-y-3">
            {filteredAlerts.map(alert => (
              <div key={alert.id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 hover:border-red-400/50 transition-all">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`px-3 py-1 text-xs font-bold rounded border ${getSeverityBadgeColor(alert.severity)}`}>
                      ...
                      {alert.status === 'escalated' && (
                        <span className="px-2 py-1 text-xs rounded bg-orange-500/10 text-orange-400 border border-orange-500/30">
                          ESCALATED
                        </span>
                      )}
                    </div>
                    <p className="text-lg font-semibold text-white mb-2">{alert.description}</p>
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-gray-400">Source: <span className="text-red-400">{alert.source}</span></span>
                  ...
                  <div className="flex flex-col gap-2 ml-4">
                    <button 
                      ...
                      className="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded-lg text-sm font-medium transition-all"
                    >
                      Create Incident
                    </button>
                    <button 
                    ...
                    >
                      ...
                      <span>{alert.status === 'escalated' ? 'Escalated' : 'Escalate to SOAR'}</span>
                    </button>
                    <button className="bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded-lg text-sm font-medium transition-all">
                      View Details
                    </button>
                  </div>
                </div>
      ...
      {activeTab === 'logs' && (
        <div className="space-y-4">
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
            ...
            <div className="bg-gray-950 border border-gray-700 rounded-lg p-4 h-96 overflow-y-auto font-mono text-sm">
              {logs.map(log => (
      ...
      {activeTab === 'correlation' && (
        <div className="space-y-4">
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-4">
              {correlations.map(corr => (
                <div key={corr.id} className="bg-gray-900 border border-gray-600 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <p className="text-lg font-semibold text-white">{corr.name}</p>
                      <p className="text-sm text-gray-400 mt-1">{corr.id} • Detected over {corr.timespan}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 text-xs font-bold rounded border ${getSeverityBadgeColor(corr.severity)}`}>
                  ...
                  <div className="bg-gray-950 border border-gray-700 rounded-lg p-3 mb-3">
                    <p className="text-sm text-gray-400 mb-2">Attack Pattern:</p>
                    <p className="text-white font-mono text-sm">{corr.pattern}</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button className="bg-red-600 hover:bg-red-700 px-3 py-1 rounded-lg text-sm font-medium transition-all">
                        Investigate
                      </button>
                      <button className="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded-lg text-sm font-medium transition-all">
                        Create Incident
                      </button>
                    </div>
                  </div>
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-4">
              {incidents.map(incident => (
                <div key={incident.id} className="bg-gray-900 border border-gray-600 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="font-mono text-sm text-cyan-400">{incident.id}</span>
                        <span className={`px-3 py-1 text-xs font-bold rounded border ${getSeverityBadgeColor(incident.severity)}`}>
                      ...
                      <p className="text-lg font-semibold text-white mb-2">{incident.title}</p>
                    ...
                    <div className="flex flex-col gap-2 ml-4">
                      ...
                      >
                        ...
                        <span>{incident.status === 'escalated' ? 'Escalated' : 'Escalate to SOAR'}</span>
                      </button>
                      <button className="bg-green-600 hover:bg-green-700 px-3 py-1 rounded-lg text-sm font-medium transition-all">
                        Resolve
                      </button>
                    </div>
                  </div>
            ...
            <div className="mb-6">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-gray-900 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-red-400"
                />
                <button className="bg-red-600 hover:bg-red-700 px-6 py-2 rounded-lg font-medium transition-all">
                  Search
                </button>
              </div>
            </div>
